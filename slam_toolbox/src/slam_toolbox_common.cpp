...
namespace slam_toolbox
{
  ...
  /*****************************************************************************/
  karto::LaserRangeFinder *SlamToolbox::getLaser(const sensor_msgs::LaserScan::ConstPtr &scan)
  /*****************************************************************************/
  {
  ...
  /*****************************************************************************/
  {
    ...
    // add the nodes and constraints relative to the Markers to the optimizer
    if (use_markers_)
    {
      MarkerVerticeMap marker_vertices = mapper->GetMarkerGraph()->GetMarkerVertices();
      MarkerVerticeMap::iterator m_vertex_map_it = marker_vertices.begin();
      for (m_vertex_map_it; m_vertex_map_it != marker_vertices.end(); ++m_vertex_map_it)
      {
        MarkerMap::iterator m_vertex_it = m_vertex_map_it->second.begin();
        for (m_vertex_it; m_vertex_it != m_vertex_map_it->second.end(); ++m_vertex_it)
        {
          if (m_vertex_it->second != nullptr)
          {
            solver_->AddNode(m_vertex_it->second);
          }
        }
      }
    ...
    karto::Sensor *pLaserSensor = dynamic_cast<karto::Sensor *>(laser);
    if (pLaserSensor)
    {
      karto::SensorManager::GetInstance()->RegisterSensor(pLaserSensor);
    ...
    if (use_markers_)
    {
      if (dataset_->GetCameras().size() < 1)
      {
        ROS_FATAL("loadSerializedPoseGraph: Cannot deserialize "
                  "dataset with no camera objects.");
        exit(-1);
      }
      ...
      karto::Sensor *pCameraSensor = dynamic_cast<karto::Sensor *>(camera);
      if (pCameraSensor)
      {
